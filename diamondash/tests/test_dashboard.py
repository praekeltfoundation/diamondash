...
class DashboardTestCase(unittest.TestCase):
    ...
    def test_slugify(self):
        ...
        self.assertEqual(slugify('SoMeThing_liKe!tHis'), 
            'something-like-this')
    ...
    def test_widget_title(self):
        ...
        """
        config = Dashboard.from_config_file(resource_filename(__name__, 
            'widget_title.yml')).config
    ...
    def test_metric_title(self):
        """
        Should use the given metric name as the metric title, or set the 
        metric title using a title key if it is explicitly specified, even
        when the two different conventions are mixed in a config file
        """
        config = Dashboard.from_config_file(resource_filename(__name__, 'metric_title.yml')).config
