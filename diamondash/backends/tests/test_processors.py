...
class SummarizersTestCase(unittest.TestCase):
    ...
    def test_last_datapoint_summarizer(self):
        summarizer_cls = processors.get_summarizer('last_datapoint')

        self.assert_summarizer(summarizer_cls(5), 3, [], [])
        self.assert_summarizer(summarizer_cls(5), 12,
           [{'x': 12, 'y': 3}], [{'x': 10, 'y': 3}])
        ...
        self.assert_summarizer(summarizer_cls(5), 8,
            [
                {'x': 8, 'y': 1},
                {'x': 12, 'y': 2},
                {'x': 21, 'y': 3},
    ...
    def test_aggregating_summarizer(self):
        ...
        summarizer_cls = processors.get_summarizer('aggregating')

        self.assert_summarizer(summarizer_cls(5, aggregator), 3, [], [])
        self.assert_summarizer(summarizer_cls(5, aggregator), 12,
           [{'x': 12, 'y': 3}], [{'x': 10, 'y': 3}])
        ...
        self.assert_summarizer(summarizer_cls(5, aggregator), 8,
            [
                {'x': 8, 'y': 1.0},
                {'x': 12, 'y': 2.0},
                {'x': 21, 'y': 3.0},
